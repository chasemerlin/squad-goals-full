...
import { electronicsList_2 } from '../../constants/Grids'
import { electronicsList_3 } from '../../constants/Grids'
import { FullImages } from '../../constants/Grids'
import { trash, star, heart } from 'ionicons/icons'
class Sliders extends Component {
    ...
    render() {
        const FullWidthImageSlider = electronicsList.map((item: any, index: number) => (
            <IonSlide key={index}>
        ...
        const ImageSlider = FullImages.map((item: any, index: number) => (
            <IonSlide key={index}>
        ...
        const Product2XSlider = this.numbers.map((number: any, pi: any) => (
            <IonSlide key={pi}>
                <IonRow className="ion-margin-bottom ion-margin-top">
                    {electronicsList_2[pi].content.map((item: any, si: any) => (
                        <IonCol size="6" key={si}>
                            <IonCard className="cardSection" margin-top>
                                <div className="heartIcon-slider" onClick={() => this.favorites(si)}>
                                    {item.checked ? <IonIcon slot="end" icon={heart} className="iconSize heartRed"></IonIcon>
                                        :
                                        <IonIcon slot="end" icon={heart} className="iconSize heart "></IonIcon>
                                    }
                                </div>
                                <img src={item.image} alt="" />
                                <IonCardContent>
        ...
        const Product3XSlider = this.numbers.map((number: any, pi: any) => (
            <IonSlide key={pi}>
                <IonRow className="ion-margin-bottom ion-margin-top">
                    {electronicsList_3[pi].content.map((item: any, si: any) => (
                        <IonCol size="4" key={si}>
                            <IonCard className="cardSection" margin-top>
                                <div className="heartIcon-slider" onClick={() => this.favorites(si)}>
                                    {item.checked ? <IonIcon slot="end" icon={heart} className="iconSize heartRed"></IonIcon>
                                        :
